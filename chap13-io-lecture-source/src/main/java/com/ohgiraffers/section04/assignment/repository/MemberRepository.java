...
public class MemberRepository {
    ...
    public MemberRepository() {
        /*설명. 회원가입 기능 추가 후 이제는 파일이 기존에 존재하면 회원 3명으로 초기화하지 X*/
        ...
        if(!file.exists()) {
            ...
            members.add(new Member(2, "user02", "pass02", 10, new String[]{"게임", "영화 감상"}, BloodType.B));
            members.add(new Member(3, "user03", "pass03", 15, new String[]{"음악 감상", "독서", "명상"}, BloodType.O));
        ...
        System.out.println("=== repository에서 읽어온 회원 정보 확인 ===");

        for (Member m: memberList) System.out.println(m);
    }

    /*설명. 회원이 담긴 ArrayList를 파일에 출력하는 기능*/
    private void saveMembers(ArrayList<Member> members) {
        ...
        try {
            ...
            /*설명. ArrayList에 저장된 회원 수만큼 파일에 출력*/
            for (Member m: members) oos.writeObject(m);

            oos.flush();                // 출력 시에는 flush() 작성 필요
        } catch (IOException e) {
            throw new RuntimeException(e);
        } finally {                     // 다른 예외 조건에서 return을 하더라도 finally 실행 이후에 return
            try {
    ...
    /*설명. 파일로부터 회원 객체들을 입력받아 memberList에 추가하는 기능*/
    private void loadMembers() {
        ...
        try {
            ...
            /*설명. 파일로부터 모든 회원 정보를 읽어와 memberList에 추가(add)*/
            while (true) {
...
//        return latestMember.getMemNo();                                     // 해당 회원의 번호
        return memberList.get(memberList.size() -1).getMemNo();
    }

    /*설명. 기존 회원(객체)에 이어서 파일 출력을 하고 추가한 객체의 수를 반환(DML 작업의 결과는 int)*/
    /*설명. 객체 출력을 할 예정인데 기존 ObjectOutputStream 대신 새로 정의한 스트림으로 회원 객체 한 명 파일 출력해서 int 반환*/
    ...
    public int deleteMember(int memNo) {
        for (int i = 0; i < memberList.size(); i++) {
            if(memberList.get(i).getMemNo() == memNo) {     // 지우려는 회원과 같은 회원 번호인 index 찾기
                ...
                /*설명. 프로그램 상에서 회원 관리하는 memberList뿐 아니라 DB(회원 파일)도 같이 적용*/
                memberList.remove(i);
